...
- Add dedicated deduplicated file CSV (ending with dedupelog.csv). Thanks for the idea Troy Larson!
- Add SHA-1 blacklisting via --hbl. Takes a text file containing one SHA-1 per line. Any files with the same SHA-1 as an entry in the file is skipped (and not logged in dedupelog.csv)
