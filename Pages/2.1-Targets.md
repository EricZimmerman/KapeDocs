...
## FollowReparsePoint
When true, KAPE will look at reparse points and collect files from them. Default when not present is FALSE. 

NOTE: In some cases this must be true for KAPE to collect files. If programs are using reparse points, such as Box or OneDrive, this property must be present and set to true for the directory to be found.

## FollowSymbolicLinks
When true, KAPE will resolve symbolic links and use the target of the link to look for files. Default when not present is FALSE. 

NOTE: In some cases this must be true for KAPE to collect files. For example, if files are locked and are sitting behind a symbolic link, this property must be present and set to true.
