...
NOTE: --ul is required on each line in order for things to continue to be linearly processed. This lets you have 5 lines in a batch file, with --ul on the first three. These would one one after the other. If the last two do not have --ul they would then be run at the same time.
