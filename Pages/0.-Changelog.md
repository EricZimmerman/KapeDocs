...
- Add dedicated log tracking skipped files (deduplicated and blacklisted).  It is a CSV ending with 'skiplog.csv'. Thanks to Troy Larson for the idea!
- Add SHA-1 blacklisting via --hbl. Takes a text file containing one SHA-1 per line. Any files with the same SHA-1 as an entry in the file is skipped (and logged in 'skiplog.csv' and ''copylog.txt')
