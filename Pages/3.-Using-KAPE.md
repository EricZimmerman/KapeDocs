...
### hbl
When given the path to a text file containing one SHA-1 per line, KAPE will not copy any files found during processing with a hash matching anything in the file. All blacklisted files are recorded in the 'skiplog.csv' file with a reason of "Blacklist". These files are also logged in 'copylog.txt'
